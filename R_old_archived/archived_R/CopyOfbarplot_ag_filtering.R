...
down_sampled <- unique(fread(paste0(outdir_dat, "/ag_seg_sites_dt.csv"))$gene_family)
...
pre_filter_ags[gene_family %chin% down_sampled, set:= "down_sampled"]
...
bp <- barplot(
  pre_filter_ags_plot,
  col = c("#243040", "#5C7086", "#8FA0B3","#C3CDD8"), 
  # for og
...
legend("topright",
       legend = c("All AGs",
                  ...
                  "After length filter"),
       fill = rev(c("#243040", "#5C7086", "#8FA0B3","#C3CDD8")), 
       border = rev(c("#243040", "#5C7086", "#8FA0B3","#C3CDD8")), 
