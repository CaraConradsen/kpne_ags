...
text(m_target, 200, "downsample target", 
     pos = 4)
...
# Unmodified piS (no hypergeometric downsampling) -------------------------
# Identify pi outliers ----------------------------------------------------
# per-gene processing 
per_core <- sites_combined_dt[, {
  Ls <- .N
  n_j      <- n
...
combined_pi_dt <- per_core[, .(
  pi_s     = mean(pi_s),
  gene_length = mean(gene_length),
  seg_total = mean(seg_s_sites)
), by = gene_family]

fwrite(combined_pi_dt, paste0(outdir_dat, "/core_age_S_dt.csv"))
