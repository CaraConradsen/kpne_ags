...
analyse_pi_desc_subtrees <- function(tree, n_genomes, internal_nodes){
  ...
  for(i in seq_along(internal_nodes)){
    ...
    # recursively collect all descendant nodes (tips AND internals)
    desc_nodes <- integer(0)
    tips <- integer(0)
    stack <- node
    while(length(stack) > 0){
      nd <- stack[1]; stack <- stack[-1]
      desc_nodes <- c(desc_nodes, nd)
      if(nd <= n_genomes) tips <- c(tips, nd) else stack <- c(stack, children[[as.character(nd)]])
    }
    freq[i] <- length(tips)
    subtree_br_len[i] <- sum(tree$edge.length[tree$edge[,2] %in% setdiff(desc_nodes, node)])
...
n_sim_trees = 500000
file_out_name = paste0(outdir_dat, "/neutral_sim_260_ntree_500K.csv")
