...
# pangenome info
data_dir = "./input_data/PIRATE_260_hybrid_chr_out/"
...
# some small contigs are id'd as 'chromosomal"?
chr_contigs <- fread(paste0(outdir_dat, "/all_pirate_anno_cogs.csv"),
                     select = c("geno_id", "seqnames", "asmbly_type", "gene"))

# REMOVE PLASMIDS
chr_contigs <- unique(chr_contigs[geno_id %chin% geno_list & asmbly_type!="plasmid" & gene=="dnaA",
                                  seqnames ])
length(chr_contigs)#214
...
# Create a fasta removing plasmids ----------------------
...
cds_fasta_gff_dt <- foreach(genome = geno_list,
  ...
  # Get PIRATE cords
  coord_fasta = coord_fasta[names(coord_fasta) %in% chr_contigs]
  ...
  # remove plasmids
  cords <- cords[seqnames %chin% chr_contigs]
  
  setorderv(cords, "start", order =1L)
  ...
  red_cords <- reduce(gr_cords, ignore.strand = TRUE)
  
  # Extract sequences
  cds_seqs <- getSeq(coord_fasta, red_cords)
  ...
  # create a new gff for adjusted fasta
  red_regions <- gr2dt(red_cords)

    # calcualte shift amount
  red_regions[, gap_to_next :=
                data.table::shift(start, type = "lead") - end - 1
  ]
  
  red_regions[, shift :=
                cumsum(data.table::shift(c(gap_to_next), fill = 0)) +
                min(start) - 1
  ]
  
  setkey(red_regions, seqnames, start, end)
  setkey(cords,  seqnames, start, end)
  ...
  adj_gr[,c("width","gap_to_next","shift",
            "i.start", "i.end", "i.strand"):= NULL]

  cat("Processed", focal_genome, "\n")
  
  adj_gr$tot_len = tot_len
  
  adj_gr$geno_id = genome
...
cds_fasta_gff_dt <- merge(cds_fasta_gff_dt, gene_families[,!"allele_name"],
                          all.x = TRUE, by = c("geno_id", "locus_tag", "gene_family"))
