...
# - Identify and annotate plasmid contigs from assembly metadata.
...
hybrid_fasta_files = list.files("./input_data/kpne_485_fasta/",
                                full.names = TRUE)

cl <- makePSOCKcluster(num_cores-6)
registerDoParallel(cl)

asmbly_dat <- foreach(i = 1:length(hybrid_fasta_files), 
                      .packages = c("mlplasmids", "Biostrings"),
                      .combine = "rbind") %dopar% {
                        ls = hybrid_fasta_files[i]
