...
png(paste0(outdir_fig,"/demography_2.0.png"),
    width = 25.7, height = 9.4, units = "cm", res = 300,
    pointsize = 12, type = "cairo")

layout(matrix(c(1:3), nrow = 1), widths = c(2.5,2.5, 2))
...
line_cols <- c(
  rgb(0.1,0.1,0.1, alpha = 0.15),   # grey        alpha = 0 (neutral reference)
  "#000000",
  "#228833",   # green
  "#AA3377",   # purple
  "#EE6677",   # red-pink
...
for (i in alpha_vals) {
  ...
  if (i == 0) {
    ...
    lines(d$freq, up, col = line_cols[a], lwd = 1.6)
    lines(d$freq, lo, col = line_cols[a], lwd = 1.6)
  }else{
  lines(d$freq, up, col = line_cols[a], lwd = 1.6, lty = "44")
  lines(d$freq, lo, col = line_cols[a], lwd = 1.6, lty = "44")
  }
}
