...
m_target = quantile(ag_m[set_1=="Y", gene_length], 
                            probs = c(0.25))

# set m target --------------------------------------------------------------

writeLines(as.character(m_target), 
...
text(m_target, 300, bquote("Q"[1]~"="~.(m_target)*"bp"), 
     pos = 4)
...
ag_genes_hypergeometric <- function(sites_dt, m_target, min_sites = 2) {
  ...
  sites_dt[, {
    ...
    if (Ls_keep < min_sites) {
      ...
      .(Ls = as.integer(Ls), Ls_keep = as.integer(Ls_keep), pi_s = NA_real_,
        thetaW_s = NA_real_, n_median = NA_real_, seg_s_sites = NA_integer_)
    } else {
...
ag_seg_sites_dt <- ag_genes_hypergeometric(syn_sites_ag_dt, m_target = m_target)
