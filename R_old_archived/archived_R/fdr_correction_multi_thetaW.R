...
theta_prod_lng_list <- list.files(paste0(outdir_dat, "/theta_w_out"),
                                  pattern ="theta_prod_lng", 
                                  full.names = TRUE, recursive = TRUE)

theta_prod_lng <- lapply(theta_prod_lng_list, function(lst){
  theta_dat <- fread(lst)
  theta_dat$theta_val <- as.numeric(gsub("theta_prod_lng_", "", 
...
fwrite(gene_pvals, paste0(outdir_dat, "/gene_pvals_mutli_theta_fdr.csv"))
